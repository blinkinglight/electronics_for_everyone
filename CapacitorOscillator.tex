...
An oscillator is usually described by either its \glossterm{wavelength} or its \glossterm{frequency}.
The wavelength of an oscillator is how many seconds it takes to go through one complete cycle.  
So, if I had lights that blinked on for one second and then off for two seconds (and continued repeating in that fashion), the wavelength would be three seconds.

The frequency of an oscillation is the number of times that the wave cycles every second, which is merely the reciprocal of the wavelength (i.e., one divided by the wavelength).
So, in the example given, since the wavelength was 3 seconds, the frequency is $\frac{1}{3}$ cycles per second.
...
What is the wavelegnth and frequency of this oscillator?

The wavelength is merely the total time it takes to go through one complete cycle.
Therefore:

\begin{align*}
\textrm{wavelenth} &= 4\textrm{seconds} + 4\textrm{seconds} \\
  &= 8\textrm{seconds}
\end{align*}

So, what is the frequency of this oscillator?  Simple, just take the reciprocal of the wavelength.  That makes the frequency $\frac{1}{8}\myhz$.
\end{exampleprob} 
...
Since the frequency is the reciprocal of the wavelength, it works the other way around as well---wavelength is the reciprocal of the frequency.
Therefore, the wavelength is simply $\frac{1}{261.6}$ seconds.
...
This gives the total wavelength of about 1 second, and a frequency of about $1\myhz$.
...
Since our time constant is 1 second, that makes the calculation really easy: $1\cdot 0.693 = 0.693\textrm{ seconds}$.
...
The total wavelength will be the total time for one cycle
...
\begin{exampleprob}
Let's say we have the basic oscillator circuit shown in Figure~\ref{figSimple555Oscillator}, but with a $2,000\myohm$ resistor for R1, a $6,000\myohm$ resistor for R2, and a $10\myuf$ capacitor for C1.  What will be the charge time, the discharge time, the wavelength, and the frequency for our oscillator?

To find this out, it is easiest to first calculate the charge and discharge times separately, and then use those to find wavelength and frequency.
...
Now that we have the charging and discharging times, we find wavelength by merely adding them together.
This means the wavelength is $0.0554 + 0.0416 = 0.097\mysec$.
